...
export function AddMasterSample() {
  ...
  const handleSubmit = async () => {
    const sampleToBeAdded = rows.map((row) => ({ name: row.name }));
    const createdSamples = await addSamples(sampleToBeAdded);
    const updatedData = [...data, ...createdSamples];
    setData(updatedData);
    setRows([]);
  };
  ...
  return (
    <Dialog>
      ...
      <DialogContent
        className="max-w-[400px] max-h-fit overflow-auto"
        aria-describedby="dialog to add new samples"
      >
        <DialogHeader>
          ...
          {rows.map((row, index) => (
            <div key={row.id} className="flex items-end justify-between">
              <div className="flex flex-col space-y-1.5">
                <Label htmlFor={`sample${index + 1}`}>Sample {index + 1}</Label>
                <Input
                  id={`sample${index + 1}`}
                  placeholder="Name of new sample"
                  className="w-[300px]"
                  value={row.name}
