...
import { addSpecifications } from "./actions";
...
export function AddMasterSpecifications() {
  ...
  const handleSubmit = async () => {
    const specificationToBeAdded = rows.map((row) => ({ name: row.name }));
    const createdSpecifications = await addSpecifications(
      specificationToBeAdded
    );
    const updatedData = [...data, ...createdSpecifications];
    setData(updatedData);
    setRows([]);
  };
    ...
    <div>
      <Dialog>
        ...
        <DialogContent className="max-w-[400px] max-h-fit overflow-auto">
          <DialogHeader>
            <DialogTitle>Add New Specifications</DialogTitle>
          </DialogHeader>
            ...
            {rows.map((row, index) => (
              <div key={row.id} className="flex items-end justify-between">
                <div className="flex flex-col space-y-1.5">
                  <Label htmlFor={`specification${index + 1}`}>
                    Specification {index + 1}
                  </Label>
                  <Input
                    id={`specification${index + 1}`}
                    placeholder="Name of new specification"
                    className="w-[300px]"
                    value={row.name}
